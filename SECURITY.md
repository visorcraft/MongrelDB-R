...
This document describes the security properties of the MongrelDB R client
and how to report vulnerabilities.

## Overview

The MongrelDB R client is an R package that talks to `mongreldb-server` over
HTTP. The client itself holds no encryption keys and stores no data at rest;
...
- Server and network errors are mapped to the typed `mongreldb_error`
  categories (`auth`, `not_found`, `constraint`, `connection`, `query`),
  not leaked as generic conditions.
- `NA`, NaN, and Infinity are rejected with a clear error rather than
  corrupting data.

## Dependency security

The MongrelDB R client depends on `curl` and `jsonlite`, both mature, widely
used CRAN packages with MIT/BSD licenses. Report dependency vulnerabilities
...
- the MongrelDB R client version, R version, and OS,
